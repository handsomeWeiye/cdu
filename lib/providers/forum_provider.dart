...
class ForumDetailsProvider with ChangeNotifier{
  ...
  Future init(int id) async{
    ...
    await CmsApi.getCommentList(refld: id.toString()).then((val){
      var jsonData = DataUtils.jsonToData(val);
      print(jsonData);
  ...
  Future getAuthorInfo(ArticleDetail articleDetail)async{
    ...
    if(articleDetail.data.extInfo ==null){
      ...
      authorInfo.authorPic =  'http://files.weiye.site/cuser/24022/2020/05/31/35d8d10a-233a-46cf-8635-4fb96f9c5b1b.jpg';
    }else{
  ...
  Future addComment(String message)async{
    await CmsApi.addComment(content: message,refld: id.toString(),token: SpUtils.getTokenSync(),).then((val){
      showToast('评论成功');
    });
    debugPrint('已经发送评论');
    notifyListeners();
  ...
  Future addArticle(String title,String content)async{
    await CmsApi.addArticle(title: title,descript: content,categoryId: '9890',content: content,extroInfo: SpUtils.getExtroInfo(),token: SpUtils.getTokenSync(),keywords: title,author: SpUtils.getUserNickName()).then((val){
      showToast('发布文章成功');
      print(val);
      var data = DataUtils.jsonToData(val);
      if(data['code']==0){
        int articleId = data['data']['id'];
